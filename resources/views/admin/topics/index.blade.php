      ...
      <section class="content">
            <div class="container-fluid">
                  <div class="row">
                        <div class="col-12">
                              <x-cards.card>
                                    <x-cards.header id='topic_wrapper'>
                                          <div class="row" style="align-items: center;">
                                                <div class="col-md-12">
                                                      <h3 class="card-title mr-5">View all topics<span
                                                ...
                                                <tbody>
                                                      @foreach ($topics as $topic)
                                                            <tr>
                                                                  ...
                                                                  <td><img src="{{ $topic->getPosterUrl() }}"
                                                                              alt="category image" width="55"></td>

                                                                  <td>{{ Str::replace('-', ' ', Str::limit($topic->name, 20)) }}
                                                                  </td>
                                                                  ...
                                                                  <td>{{ $topic->series_count }}</td>

                                                                  <td>{{ $topic->getEpisodeCount() }}</td>
                                                                  ...
                                                                  <td>{{ $topic->created_at->format('M d/y') }}
                                                                  </td>

                                                                  <td>

                                                                        {{-- edit the topic --}}
                                                                        <a href="{{ route('admins.topics.edit', $topic) }}"
                                                                        ...
                                                                        @if (!$topic->activation)
                                                                              {{-- not active --}}
                                                                              {{-- activation the topic --}}
                                                                              <form class='d-inline' method='post'
                                                                                    action='{{ route('admins.topics.activation', $topic) }}'>
                                                                                    @csrf
                                                                                    @method('POST')
                                                                                    <button type="submit"
                                                                                          data-toggle="tooltip"
                                                                                          data-placement="bottom"
                                                                                          title="activation topic"
                                                                        ...
                                                                        {{-- delete category --}}
                                                                        <form class='d-inline' method='post'
                                                                              action='{{ route('admins.topics.destroy', $topic) }}'>
                                                                              @csrf
                                                                              @method('DELETE')
                                                                              <button type="submit"
                                                                                    data-toggle="tooltip"
      ...
      @section('scripts')
            <script>
                  $(function() {
                        ...
                        }).buttons().container().appendTo('#topic_wrapper .col-md-5:eq(0)');
                  });
            ...
            <x-alerts.delete permission="topics_delete" />
            <!-- make post active or not -->
            <x-alerts.activation permission='topics_update' />
