<x-forms.form action="{{ route('admins.topics.update', $topic) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="row">
                  ...
                  <x-cards.card class="card-outline card-info">
                        ...
                        <x-cards.body>
                              ...
                              <x-forms.form-group>
                                    ...
                                    <x-forms.input type='text' name='name' value="{{ old('name', str_replace('-', ' ', $topic->name)) }}"
                              ...
                              <div class="text-center">
                                    <img id="output" src="{{ $topic->getPosterUrl() }}" class='mt-3'
                                          style="max-width: 100%;" />
                              </div>
                                    ...
                                    <select name="playlist_category_id" class="form-control select2">
                                          ...
                                          @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                      {{ $topic->category->name == $category->name ? 'selected' : '' }}>
                              ...
                              <x-forms.form-group class="mb-4">
                                    ...
                                    <select class="form-control select2" name='activation' style="width: 100%;">
                                          <option value="1" {{ $topic->activation == '1' ? 'selected' : '' }}>
                                                Active</option>
                                          <option value="0" {{ $topic->activation == '0' ? 'selected' : '' }}>Not
                                                active</option>
                                    </select>
                              </x-forms.form-group>

                              <x-forms.submit class="btn-success w-100"> <i class="fa-fw fas fa-check"></i> Update
                              </x-forms.submit>
