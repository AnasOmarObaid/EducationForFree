      ...
      <section class="content">
            <div class="container-fluid">
                  <div class="row">
                        <div class="col-12">
                              <x-cards.card>
                                    <x-cards.header id='series_wrapper'>
                                          <div class="row" style="align-items: center;">
                                                <div class="col-md-12">
                                                      <h3 class="card-title mr-5">View all series<span
                                                            ...
                                                            <select name="topics[]"
                                                                  ...
                                                                  @foreach ($topics as $topic)
                                                                        <option value="{{ $topic->name }}"
                                                                              {{ is_array(request('topics')) && in_array($topic->name, request('topics')) ? 'selected' : '' }}>
                                                                              {{ $topic->name }}</option>
                                                                  @endforeach
                                                            </select>
                                                ...
                                                <tbody>
                                                      @foreach ($serieses as $series)
                                                            <tr>
                                                                  ...
                                                                  <td><img src="{{ $series->getPosterUrl() }}"
                                                                              alt="series image" width="55"></td>

                                                                  <td>{{ Str::replace('-', ' ', Str::limit($series->name, 20)) }}
                                                                  </td>

                                                                  <td>{{ Str::limit($series->description, 30) }}
                                                                  </td>

                                                                  <td>{{ $series->user->username }}</td>

                                                                  <td>{{ $series->topic->category->name }}</td>

                                                                  <td>{{ $series->topic->name }}</td>
                                                                  ...
                                                                  <td>{{ $series->created_at->format('M d/y') }}
                                                                  </td>
                                                                  ...
                                                                  <td>
                                                                        ...
                                                                        @if (!$series->activation)
                                                                              {{-- not active --}}
                                                                              {{-- activation the series --}}
                                                                              <form class='d-inline' method='post'
                                                                                    action='{{ route('admins.series.activation', $series) }}'>
                                                                                    @csrf
                                                                                    @method('POST')
                                                                                    <button type="submit"
                                                                                          data-toggle="tooltip"
                                                                        ...
                                                                        {{-- delete bits --}}
                                                                        <form class='d-inline' method='post'
                                                                              action='{{ route('admins.series.destroy', $series) }}'>
                                                                              @csrf
                                                                              @method('DELETE')
                                                                              <button type="submit"
                                                                                    data-toggle="tooltip"
      ...
      @section('scripts')
            <script>
                  $(function() {
                        ...
                        }).buttons().container().appendTo('#series_wrapper .col-md-5:eq(0)');
                  });
            ...
            <x-alerts.delete permission="series_delete" />
            <!-- make series active or not -->
            <x-alerts.activation permission='series_update' />
